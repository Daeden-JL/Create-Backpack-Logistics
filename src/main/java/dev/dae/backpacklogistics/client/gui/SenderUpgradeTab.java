...
public class SenderUpgradeTab extends UpgradeSettingsTab<SenderUpgradeContainer> {
	private final FilterLogicControl<FilterLogic, FilterLogicContainer<FilterLogic>> filterLogicControl;

	public SenderUpgradeTab(SenderUpgradeContainer upgradeContainer, Position position, StorageScreenBase<?> screen,
		...
		addHideableChild(new NumberSelector(new Position(x + 3, y + 36), 96,
				"gui.backpack_logistics.upgrades.sender.upper_threshold",
				upgradeContainer::getUpperThreshold, upgradeContainer::setUpperThreshold, 0, SenderUpgradeWrapper.MAX_THRESHOLD));

		addHideableChild(new NumberSelector(new Position(x + 3, y + 56), 96,
				"gui.backpack_logistics.upgrades.sender.lower_threshold",
				upgradeContainer::getLowerThreshold, upgradeContainer::setLowerThreshold, 0, SenderUpgradeWrapper.MAX_THRESHOLD));

		addHideableChild(new TextWidget(new Position(x + 3, y + 78), 96,
				() -> Component.translatable("gui.backpack_logistics.upgrades.sender.address_label")));
		...
		addressBox.setValueWithoutNotification(upgradeContainer.getTargetAddress());
		addressBox.setUnfocusedEmptyHint("-");
		addressBox.setResponder(upgradeContainer::setTargetAddress);
		addHideableChild(addressBox);

		addHideableChild(new TextWidget(new Position(x + 3, y + 108), 96,
				() -> Component.translatable("gui.backpack_logistics.upgrades.sender.filter_label")));
